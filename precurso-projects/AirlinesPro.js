...
const askUsername = () => {
  let askUsernamePrompt = prompt(
    "Bienvenido al sistema, por favor introduzca su nombre de usuario: "
  );
...
const showFlights = () => {
  ...
  flightsDuplicate.forEach((flight) => {
    console.log(
      `El vuelo con origuen: ${flight.from}, y destino: ${flight.to} tiene un coste de ${flight.cost}€ y ${flight.scale}.`
    );
    console.log(
      "------------------------------------------------------------------------------------------------------------"
    );
...
const flightsWithScale = () => {
  ...
  flights.forEach((flight) => {
    if (flight.scale) {
      console.log(
        `El vuelo con origuen: ${flight.from}, y destino: ${flight.to} tiene escala.`
      );
      console.log(
        "--------------------------------------------------------------------"
      );
...
const lastFlightOfDay = () => {
  ...
  newArray.forEach((flight) => {
    console.log(
      `El vuelo con id: ${flight.id}, tiene como destino: ${flight.to}.`
    );
    console.log("-------------------------------------------------------");
  });
  console.log("\n");
...
const askRoleUser = () => {
  ...
  do {
    if (
      askRoleUserPrompt.toLowerCase() === "user" ||
      askRoleUserPrompt.toLowerCase() === "admin"
    ) {
      flag = true;
    } else {
      askRoleUserPrompt = prompt(
        "Por favor indique alguna de las opciones validas (user o admin): "
      );
    }
  } while (flag === false);
};
...
const actionsAllowedForAdmin = () => {
  ...
  if (askRoleUserPrompt === "admin") {
    actionsAdmin = prompt(
      "Con su rol podra crear o eliminar un vuelo. Por favor introduzca la palabra crear o eliminar: "
    );
    flag = false;
    do {
      if (
        actionsAdmin.toLowerCase() === "crear" ||
        actionsAdmin.toLowerCase() === "eliminar"
      ) {
        flag = true;
      } else {
        actionsAdmin = prompt(
          "Por favor indique alguna de las opciones validas (crear o eliminar): "
        );
      }
    } while (flag === false);
  }
  ...
  switch (actionsAdmin) {
    case "crear":
      ...
      do {
        ...
        while (
          repeatCreationFlight.toLowerCase() !== "si" &&
          repeatCreationFlight.toLowerCase() !== "no"
        ) {
    ...
    case "eliminar":
      ...
      do {
        let flightDelete = prompt(
          "Ingrese el numero de ID del vuelo que desea eliminar: "
        );
        while (
          flightDelete === "" ||
          isNaN(flightDelete) ||
          flightDelete.match(/ /g)
        ) {
          flightDelete = prompt(
            "Por favor introduzca un valor de tipo number: "
          );
        }
        ...
        while (
          repeatDeleteFlight.toLowerCase() !== "si" &&
          repeatDeleteFlight.toLowerCase() !== "no"
        ) {
...
const checkNewFlights = () => {
  flightDestination = prompt(
    "Por favor introduzca un valor para el destino del vuelo: "
  );
  while (
    flightDestination === "" ||
    Number(flightDestination) ||
    flightDestination.match(/ /g)
  ) {
    flightDestination = prompt(
      "Por favor introduzca un valor de tipo string: "
    );
  }

  flightOrigin = prompt(
    "Por favor introduzca un valor para el origen del vuelo: "
  );
  while (
    flightOrigin === "" ||
    Number(flightOrigin) ||
    flightOrigin.match(/ /g)
  ) {
    flightOrigin = prompt("Por favor introduzca un valor de tipo string: ");
  }

  flightPrice = prompt(
    "Por favor introduzca un valor para el precio del vuelo: "
  );
  ...
  flightScale = prompt(
    "Por favor introduzca un valor (true/false) para la escala del vuelo: "
  );
  flag = false;
  do {
    if (
      flightScale.toLowerCase() === "true" ||
      flightScale.toLowerCase() === "false"
    ) {
      flag = true;
    } else {
      flightScale = prompt(
        "Por favor indique alguna de las opciones validas (true o false): "
      );
    }
  } while (flag === false);
...
const actionsAllowedForUser = () => {
  if (askRoleUserPrompt === "user") {
    let actionUser = prompt(
      "Con su rol podra buscar por precio los vuelos cuyo importe sea igual o inferior al valor introducido. Por favor introduzca el precio: "
    );
    ...
    console.log("\n");
    console.log(
      `Los vuelos con un importe inferior o igual a ${actionUser}€ son: `
    );
