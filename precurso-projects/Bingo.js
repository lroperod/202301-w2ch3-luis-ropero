...
const askUsername = () => {
    ...
    console.log("\n");
    console.log("El juego cuenta con un sistema de puntacion que depende en funcion del numero de turnos que necesite para cantar bingo:");
    console.log("    * Si consigue Bingo antes de la ronda 16 obtendra 1500 puntos.");
    console.log("    * Si consigue Bingo antes de la ronda 25 obtendra 1000 puntos.");
...
const newBingoCard = ()=>{
    ...
    bingoCard = arrayRandomNumbers.map(number=>({number: number, matched: false}));
    return bingoCard;
}
...
const changeBingoCard = ()=>{
    ...
    console.log("### El carton que se ha generado aleatoriamente con el que usted va a jugar es: ###");
    console.log("\n");
    console.log(`Linea 1:  ${bingoCard[0].number}, ${bingoCard[1].number}, ${bingoCard[2].number}, ${bingoCard[3].number}, ${bingoCard[4].number}`);
    console.log(`Linea 2:  ${bingoCard[5].number}, ${bingoCard[6].number}, ${bingoCard[7].number}, ${bingoCard[8].number}, ${bingoCard[9].number}`);
    console.log(`Linea 3:  ${bingoCard[10].number}, ${bingoCard[11].number}, ${bingoCard[12].number}, ${bingoCard[13].number}, ${bingoCard[14].number}`);
    console.log("\n");
    
    playThisCard = prompt("Desea jugar con este carton (si/no): ");
    while (playThisCard.toLowerCase() !== "si" && playThisCard.toLowerCase() !== "no"){
...
const checkSuccessBingoCard = () =>{
    ...
    if(bingoCard.number=== numberToPlay){
        ...
        console.log("Enhorabuena el numero que ha salido se encuentra en su carton!!!");
        console.log("\n");
    }});
}
...
if(!line1Flag){
    const firstFiveNumberAreTrue = bingoCard.slice(0,5).every(bingoCard => bingoCard.matched === true);
    if(firstFiveNumberAreTrue){
        console.log("LINEA!!! Enhorabuena ha acertado todos los numeros de la línea 1.");
        line1Flag=true;
    }  
}

if(!line2Flag){
    const seconddFiveNumberAreTrue = bingoCard.slice(5,10).every(bingoCard => bingoCard.matched === true);
    if(seconddFiveNumberAreTrue){
        console.log("LINEA!!! Enhorabuena ha acertado todos los numeros de la línea 2.");
        line2Flag=true;
    }  
}

if(!line3Flag){
    const thirdFiveNumberAreTrue = bingoCard.slice(10,15).every(bingoCard => bingoCard.matched === true);
    if(thirdFiveNumberAreTrue){
        console.log("LINEA!!! Enhorabuena ha acertado todos los numeros de la línea 3.");
        line3Flag=true;
    }  
}}
...
const checkBingo = ()=>{
    const allNumbersAreTrue = bingoCard.slice(0,15).every(bingoCard=> bingoCard.matched === true);
    if(allNumbersAreTrue){
        console.log("BINGO!!! Enahorabuena ha acertado todos los numeros del carton.");
        console.log("\n");
        endGame = true;
        nextTurn ==="no";
    }
...
const playNextTurn = ()=>{
    while(nextTurn === "si" && endGame=== false){
        ...
        while (nextTurn.toLowerCase() !== "si" && nextTurn.toLowerCase() !== "no"){
